...
#define AHB_DA2_BASE 	*(uint32_t*) 0x54000000
...
int main(void) {
  ...
  while(1)
  {
		AHB_DA2_BASE = sineLookupTable[idx];
		while(sample < 17)	// spin loop to control sample rate, writes new sample to DA2 every 
		{
			sample = sample + 1; 
		}
		sample = 0;
